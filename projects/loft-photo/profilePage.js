...
export default {
  async setUser(user) {
    ...
    const photos = document.getPhotos(user.id);
  ...
  handleEvents() {
    document.querySelector('.component-user-photos').addEventListener('click', async () => {
      if (e.target.classList.contains('component-user-photo')) {
        ...
        mainPage.setFriendAndPhoto(this.ures, parseInt(photoId), size.url);
        pages.openPage('main');
      }
    });
